...
练度流程列表['安可'] =
{
    ...
    buff_列表: {
        ...
        '深塔-10抗': { 属性: { 抗性: -10 } },
        '深塔-6增伤': { 属性: { 增伤: 6 }, 标签: '没升', 描述: '角色释放声骸技能时，全队全属性伤害提升6%，最多叠加4层，同角色的同名声骸技能无法重复触发该效果。叠加至4层时，全队暴击伤害提升36%。' },
    ...
    流程: function () {
        ...
        this.add_action('test 变奏 10154', 1 / 1e4, '没升')
        this.add_action('变奏', 1.9881)

        this.add_buff("安可-10增伤")

        this.add_action('test E3 6954', 0.6332 / 1e4, '共鸣技能', '没升')
        ...
        // this.add_action('test A1 15616', 0.9018 / 1e4, '普攻', '没升')
        // this.add_action('test A2 9766', 0.5640 / 1e4, '普攻', '没升')
        // this.add_action('test A3 11427', 0.6599 / 1e4, '普攻', '没升')
        this.add_action('test A4 33594', 1.9401 / 1e4, '普攻', '没升')
        ...
        this.add_buff("掣傀之手-12攻-2/4", 2)
        this.add_action('test 延奏 10326', 1.7676 / 2 / 1e4, '没升')

        this.add_buff("掣傀之手-12攻-2/4", -2)
        this.add_buff("掣傀之手-12攻-2/4", -2)
        this.add_buff("深塔-6增伤", 1)

        this.add_action('test 延奏 9216', 1.7676 / 2 / 1e4, '没升')

        this.add_action('test Q 12639', 2.4240 / 2 / 1e4, '没升')
        this.add_action('test Q 14745', 2.8280 / 2 / 1e4, '没升')
        ...
        // this.add_action('test A2 9766', 0.5640 / 1e4, '普攻', '没升')
        // this.add_action('test A3 11427', 0.6599 / 1e4, '普攻', '没升')
        this.add_action('test A4 31893', 1.9401 / 1e4, '普攻', '没升')

        this.add_action('AAAA AAA AAAA', 0.9018 * 2 * 3 + 0.5640 * 3 * 3 + 0.6599 * 4 * 3 + 1.9401 * 3 * 2, '普攻')

        // this.add_action('test E ', 0.6618 / 1e4, '共鸣技能', '没升')
        // this.add_action('test E ', 2.93 / 1e4, '共鸣技能', '没升')
        this.add_action('E ', 0.7661 * 8, '共鸣技能')

        this.add_action('test 延奏 33594', 1.7676 / 1e4, '没升')
        this.add_action('延奏', 1.7676 * 4)
